...
def _get_system_agents() -> List[Dict[str, Any]]:
    """
    12 AT·OM System Agents for Universe and Profile Mapping.

    These agents run automatically to:
    1. UNIVERSE MAPPING: Build and maintain the universal AT·OM knowledge graph
    2. PROFILE MAPPING: Generate personalized mappings for each user identity

    All agents respect R&D Rules:
    - Rule #1: Suggestions only, humans approve
    - Rule #6: Full traceability
    """
    return [
        # ═══════════════════════════════════════════════════════════════════════
        # UNIVERSE MAPPING AGENTS (Global AT·OM Knowledge Graph)
        # ═══════════════════════════════════════════════════════════════════════
        {
            "name": "atom_universe_gematria_mapper",
            "display_name": "Universe Gematria Mapper",
            "description": "Generates gematria signatures for universal AT·OM nodes. Maps sacred numerology across civilizations (Maya, Egypt, Kabbalah, Yi-King).",
            "capabilities": ["analysis", "data_processing"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "universe", "layer": "gematria"}
        },
        {
            "name": "atom_universe_vibration_engine",
            "display_name": "Universe Vibration Engine",
            "description": "Calculates vibrational frequencies (444Hz base) and harmonic signatures for universal nodes. Generates spiral coordinates using phi ratio.",
            "capabilities": ["analysis", "data_processing"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "universe", "layer": "vibration"}
        },
        {
            "name": "atom_universe_causal_nexus",
            "display_name": "Universe Causal Nexus",
            "description": "Suggests causal links between universal AT·OM nodes. Evidence-gated: high-confidence links require provenance.",
            "capabilities": ["analysis", "recommendation"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "universe", "layer": "causal"}
        },
        {
            "name": "atom_universe_resonance_engine",
            "display_name": "Universe Resonance Engine",
            "description": "Detects cross-dimension resonances in universal knowledge. Maps sensory, psychological, resource, concept, and logistics dimensions.",
            "capabilities": ["analysis", "recommendation"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "universe", "layer": "resonance"}
        },
        {
            "name": "atom_universe_harmonic_sync",
            "display_name": "Universe Harmonic Synchronizer",
            "description": "Identifies 'gold nodes' with multi-dimensional alignment in universal graph. Detects patterns divisible by 9 (sacred geometry).",
            "capabilities": ["analysis", "scoring"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "universe", "layer": "harmonic"}
        },
        {
            "name": "atom_universe_cosmology_bridge",
            "display_name": "Universe Cosmology Bridge",
            "description": "Bridges between cosmological systems: Tzolkin Maya, Yi-King hexagrams, Kabbalah Sephiroth, Chakra system, Atlantean codes.",
            "capabilities": ["analysis", "data_processing", "recommendation"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "universe", "layer": "cosmology"}
        },
        # ═══════════════════════════════════════════════════════════════════════
        # PROFILE MAPPING AGENTS (Personal AT·OM for each User)
        # ═══════════════════════════════════════════════════════════════════════
        {
            "name": "atom_profile_identity_mapper",
            "display_name": "Profile Identity Mapper",
            "description": "Maps user identity to AT·OM signatures. Calculates personal gematria from name, birthdate, and key life data.",
            "capabilities": ["analysis", "data_processing"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "profile", "layer": "identity"}
        },
        {
            "name": "atom_profile_vibration_calculator",
            "display_name": "Profile Vibration Calculator",
            "description": "Generates personal vibrational profile for user. Includes base frequency, harmonic overtones, and phi-spiral coordinates.",
            "capabilities": ["analysis", "data_processing"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "profile", "layer": "vibration"}
        },
        {
            "name": "atom_profile_resonance_finder",
            "display_name": "Profile Resonance Finder",
            "description": "Finds resonances between user profile and universal AT·OM nodes. Suggests connections based on dimensional alignment.",
            "capabilities": ["analysis", "recommendation"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "profile", "layer": "resonance"}
        },
        {
            "name": "atom_profile_thread_integrator",
            "display_name": "Profile Thread Integrator",
            "description": "Integrates user threads and activities into personal AT·OM map. Builds causal graph of user's journey.",
            "capabilities": ["analysis", "data_processing"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "profile", "layer": "threads"}
        },
        {
            "name": "atom_profile_sphere_harmonizer",
            "display_name": "Profile Sphere Harmonizer",
            "description": "Harmonizes user's 9 spheres with AT·OM universal patterns. Detects imbalances and suggests alignment opportunities.",
            "capabilities": ["analysis", "recommendation"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "profile", "layer": "spheres"}
        },
        {
            "name": "atom_profile_evolution_tracker",
            "display_name": "Profile Evolution Tracker",
            "description": "Tracks user's AT·OM evolution over time. Records vibrational shifts, resonance changes, and growth patterns.",
            "capabilities": ["analysis", "data_processing", "summarization"],
            "requires_human_gate": False,
            "is_system_agent": True,
            "auto_activate": True,
            "scope": {"domain": "profile", "layer": "evolution"}
        },
    ]


# Total including system agents: 226 + 12 = 238
SYSTEM_AGENTS_COUNT = 12


def get_all_predefined_agents() -> List[Dict[str, Any]]:
    """
    Get all 238 predefined agents (226 sphere + 12 system).
    ...
    sphere_generators = {
        ...
        SphereType.SYSTEM: _get_system_agents,  # AT·OM System Agents
    }
    ...
    for sphere_type, generator in sphere_generators.items():
        ...
        for agent_def in sphere_agents:
            ...
            agent_def.setdefault("is_system_agent", sphere_type == SphereType.SYSTEM)
            agent_def.setdefault("auto_activate", sphere_type == SphereType.SYSTEM)
            agent_def.setdefault("description", f"{agent_def['display_name']} for {sphere_type.value} sphere")
            agents.append(agent_def)

    # Total: 226 sphere agents + 12 system agents = 238
    expected_total = TOTAL_AGENTS + SYSTEM_AGENTS_COUNT
    assert len(agents) == expected_total, f"Expected {expected_total} agents, got {len(agents)}"

    return agents


def get_system_agents() -> List[Dict[str, Any]]:
    """Get only the 12 AT·OM system agents."""
    return _get_system_agents()


def get_universe_mapping_agents() -> List[Dict[str, Any]]:
    """Get the 6 Universe Mapping agents."""
    return [a for a in _get_system_agents() if a.get("scope", {}).get("domain") == "universe"]


def get_profile_mapping_agents() -> List[Dict[str, Any]]:
    """Get the 6 Profile Mapping agents."""
    return [a for a in _get_system_agents() if a.get("scope", {}).get("domain") == "profile"]
...
class AgentRegistryService:
    ...
    async def seed_agents(self) -> int:
        """
        Seed the database with all 238 predefined agents (226 sphere + 12 system).

        Returns number of agents created.
        """
        # Check if already seeded (use new total)
        expected_total = TOTAL_AGENTS + SYSTEM_AGENTS_COUNT
        ...
        for agent_data in agents_data:
            ...
            agent = Agent(
                ...
                is_system_agent=agent_data.get("is_system_agent", False),
                auto_activate=agent_data.get("auto_activate", False),
            )
            self.db.add(agent)
            created += 1

        await self.db.flush()
        logger.info(f"Seeded {created} agents (including {SYSTEM_AGENTS_COUNT} system agents)")
    ...
    async def get_system_agents(self) -> List[Agent]:
        """Get all AT·OM system agents."""
        result = await self.db.execute(
            select(Agent).where(Agent.is_system_agent == True).order_by(Agent.name)
        )
        return list(result.scalars().all())

    async def get_auto_activate_agents(self) -> List[Agent]:
        """Get agents that should auto-activate on identity creation."""
        result = await self.db.execute(
            select(Agent).where(
                and_(
                    Agent.auto_activate == True,
    ...
    async def activate_system_agents_for_identity(self, identity_id: UUID) -> int:
        """
        Auto-activate all system agents for a new identity.

        Called when a new identity is created to ensure AT·OM mapping
        agents are ready to work immediately.

        Returns number of agents activated.
        """
        auto_agents = await self.get_auto_activate_agents()
        activated = 0

        for agent in auto_agents:
            # Check if already configured
            existing = await self.get_user_agent_config(identity_id, agent.id)
            if existing:
                continue
        ...
        await self.db.flush()
        logger.info(f"Auto-activated {activated} system agents for identity {identity_id}")

        return activated

    async def get_universe_mapping_agents(self) -> List[Agent]:
        """Get the 6 Universe Mapping agents."""
        result = await self.db.execute(
            select(Agent).where(
                and_(
                    Agent.is_system_agent == True,
                    Agent.scope.contains({"domain": "universe"})
                )
            )
        )
        return list(result.scalars().all())

    async def get_profile_mapping_agents(self) -> List[Agent]:
        """Get the 6 Profile Mapping agents."""
        result = await self.db.execute(
            select(Agent).where(
                and_(
                    Agent.is_system_agent == True,
                    Agent.scope.contains({"domain": "profile"})
                )
            )
        )
        return list(result.scalars().all())
...
__all__ = [
    ...
    "get_system_agents",
    "get_universe_mapping_agents",
    "get_profile_mapping_agents",
    "AGENT_DISTRIBUTION",
    "TOTAL_AGENTS",
    "SYSTEM_AGENTS_COUNT",
]
